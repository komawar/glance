...
class TaskImportExecutor(async.TaskExecutorInterface):
    ...
    def run(self, task_proxy):
        ...
        LOG.info(msg)
        #TODO: add a pool of threads so that we do not overlimit
        importer_module = importutils.import_module('glance.common.scripts.filesystem_filesystem_importer')
        importer = importer_module.TaskFilesystemToFilesystemImportExecutor(self.request, self.gateway)
        eventlet.spawn_n(importer.execute, task_proxy)
