...
class TestTaskImportExecutor(base.IsolatedUnitTest):
    ...
    def setUp(self):
        ...
        self.import_executor = import_executor.\
            TaskImportExecutor(self.request, gateway=fake_gateway)
...
#            called['execute'] = True
#
#        self.stubs.Set(import_executor.TaskImportExecutor,
...
#        self.assertNotEqual(count, 0)
    ...
    def test_execute(self):
        with mock.patch.object(self.import_executor, 'unpack_task_input') \
            as mock_unpack_task_input:
            mock_unpack_task_input.return_value = {'image_properties': {}}
            with mock.patch.object(self.import_executor, 'create_image'):
                with mock.patch.object(self.import_executor,
    ...
    def test_invalid_input_from(self):
        ...
        self.assertRaises(exception.BadStoreUri,
                          self.import_executor.check_valid_location,
                          input)

    def test_valid_input_from(self):
        input = {
            'image_properties': {},
            'import_from': 'file:///valid',
        }
        self.assertEqual(self.import_executor.check_valid_location(input),
                         '/valid')
    ...
    def test_set_image_data(self):
        ...
        with open(uri, 'w') as fptr:
            fptr.write("data")
        self.import_executor.set_image_data(fake_image, uri)
