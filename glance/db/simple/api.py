...
def is_task_visible(context, task):
...
def task_create(context, task_values):
    ...
    if task_id in DATA['tasks']:
        raise exception.Duplicate()
    for key in ('type', 'status', 'input'):
        if key not in task_values:
            raise exception.Invalid('%s is a required attribute' % key)

    allowed_keys = set(['id', 'type', 'status', 'input', 'result', 'owner',
                        'message', 'expires_at', 'created_at', 'updated_at',
                        'deleted_at', 'deleted'])

    incorrect_keys = set(task_values.keys()) - allowed_keys
    if incorrect_keys:
        raise exception.Invalid(
            'The keys %s are not valid' % str(incorrect_keys))
...
def _task_get(context, task_id, force_show_deleted=False):
    try:
        ...
        LOG.info(_('Could not find task %s') % task_id)
        raise exception.NotFound()

    if task['deleted'] and not (force_show_deleted or context.show_deleted):
        LOG.info(_('Unable to get deleted task'))
        raise exception.NotFound()

    if not is_task_visible(context, task):
